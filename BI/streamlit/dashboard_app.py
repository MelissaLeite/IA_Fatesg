#Discente: Melissa Cunha Leite
#Disciplina: Business Intelligence IA2

#ESTRUTURA INICIAL

#Importando as bibliotacas necessárias

import streamlit as st          #biblioteca para gráficos e dashboards
import pandas as pd             #bibliotca para manipulação de dados
import matplotlib.pyplot as plt #biblioteca para gráficos
from pathlib import Path

#CARREGAR OS DADOS

#carregar arquivo de dados csv
df = pd.read_csv(
    "financiamentoPecuaria.csv",
    sep=";",
    decimal=",",
    thousands="."
)
...
#retirar a segunda coluna do dataframe
...
#trocar todos os '-' por 0
df = df.replace('-', 0)
df.dtypes #verificar os tipos de dados das colunas
print(df.head())
...
df.iloc[:, 1:] = df.iloc[:, 1:].apply( #seleciona todas as colunas, exceto a primeira
    lambda col: pd.to_numeric(
        col.str.replace('.', '', regex=False).str.replace(',', '.', regex=False), #primeira parte remove os pontos. segunda parte troca a vírgula por ponto
        errors='coerce'
        )
    )
...
#calcula a média das colunas a partit da segunda (colunas de 2014 a 2024)
#para cada linha (localidade)
df["Md_Invest"] = df.iloc[:, 1:].mean(axis=1)

#arredondar para duas casas decimais
df["Md_Invest"] = pd.to_numeric(df["Md_Invest"], errors="coerce").round(2)


# Inserir uma linha com a média de investimento para cada ano
# 1. Calcula a média das colunas de investimento (excluindo a coluna 'Localidade') #df.iloc[:, 1:] seleciona todas as colunas a partir da segunda (índices dos anos)
...
#iniciar o dashboard
# Configuração do layout da tabela na página st.set_page_config(layout="wide")
...
df["Md_Invest"] = dados.mean(axis=1).round(2)
...
with col1:
    ...
    if localidade_selecionada == "Todos":
        ...
        # Certifique-se de que a seleção e os dados tenham exatamente o mesmo formato
        df.local = df[df["Localidade"] == localidade_selecionada]
        # Exibir dataframe para conferir se está correto (debug) 
        st.write(f"Dados filtrados para {localidade_selecionada}:")
        st.dataframe(df_local)
        if not df_local.empty:
...
with col3:
    ...
    if localidade_selecionada != "Todos" and not df_local.empty:
        ...
        df_comparacao = pd. DataFrame({
            "Ano": colunas_anos, localidade_selecionada: investimentos_local.values,
        ...
        # Exibe o gráfico st.bar_chart(df_comparacao)
    else :
